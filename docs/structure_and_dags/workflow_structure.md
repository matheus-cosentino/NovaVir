# 🇬🇧 NovaVir Pipeline Structure (English)

Below is the detailed documentation of the directory and file structure of the NovaVir pipeline, mapped based on the repository organization.
...
*   **`NovaVir.sh`**: The main wrapper script of the pipeline. It is responsible for receiving user parameters via the terminal, activating Conda environments, creating dynamic passing configurations, and triggering the Snakemake executor.
...
*   **`run_overrides.yaml`**: Dynamic configuration file generated by `NovaVir.sh` at each run. It temporarily overrides key variables for Snakemake (such as absolute paths, step toggles, and working directory names).
...
# 🇧🇷 Estrutura do Pipeline NovaVir (Português)

Abaixo está a documentação detalhada da estrutura de diretórios e arquivos do pipeline NovaVir, mapeada com base na organização do repositório exibida pelo comando `tree`.

## 📂 Diretório Raiz (`/`)

*   **`NovaVir.sh`**: O script principal (wrapper) do pipeline. É responsável por receber os parâmetros do usuário pelo terminal, ativar os ambientes Conda, criar as configurações dinâmicas de passagem e acionar o executor do Snakemake.
...
*   **`run_overrides.yaml`**: Arquivo de configuração dinâmico gerado pelo `NovaVir.sh` a cada rodada. Ele subscreve temporariamente variáveis chaves para o Snakemake (como caminhos absolutos, ativação ou não de etapas, e nome do *workdir* de arrasto).
...
# 🇫🇷 Structure du Pipeline NovaVir (Français)

Vous trouverez ci-dessous la documentation détaillée de l'arborescence et des fichiers du pipeline NovaVir, cartographiée en fonction de l'organisation du dépôt.

## 📂 Répertoire Racine (`/`)

*   **`NovaVir.sh`**: Le script principal (wrapper) du pipeline. Il est chargé de recevoir les paramètres de l'utilisateur via le terminal, d'activer les environnements Conda, de créer des configurations de passage dynamiques et de déclencher l'exécuteur Snakemake.
...
*   **`run_overrides.yaml`**: Fichier de configuration dynamique généré par `NovaVir.sh` à chaque exécution. Il surcharge temporairement les variables clés pour Snakemake (telles que les chemins absolus, l'activation des étapes et les noms de répertoires de travail).
